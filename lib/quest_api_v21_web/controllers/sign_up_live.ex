defmodule QuestApiV21Web.SignUpLive do
  ...
  def render(assigns) do
    ...
    <div class="w-full flex justify-center mt-8">
      <a href="/sign_in" class="text-white absolute">
        Have an account? - <span class="underline underline-offset-1">sign in</span>
      </a>
    </div>
        ...
        <div class="px-8 pb-8  pt-6 mt-8 bg-transparent max-w-sm w-80 rounded-xl shadow-md mx-auto">
          ...
          <div class="mb-4">
            ...
            <%= text_input(f, :name,
              ...
              placeholder: "Name",
              class: "w-full pl-4 p-3 border rounded-full",
          ...
          <div class="mb-4">
            ...
            <%= email_input(f, :email,
              ...
              placeholder: "Email",
              class:
                "w-full pl-4 p-3 border rounded-full  #{if @email_valid, do: "focus:outline-none focus:ring-2 focus:ring-green-500 border-1 focus:border-green-500 border-green-500"}",
          ...
          <div class="mb-6">
            ...
            <%= password_input(f, :password,
              ...
              class:
                "w-full pl-4 p-3 border rounded-full #{password_input_class(@password_strength)}",
          ...
          <div class="mt-3">
            ...
            <%= submit("Sign Up",
              class:
                "border-2 border-brand text-lg w-full py-2 px-4 bg-white font-medium text-slate-900 rounded-lg
                  #{if !@form_valid, do: "opacity-50 cursor-not-allowed bg-brand ", else: "white-glow drop-shadow-2xl"}",
              disabled: !@form_valid
            ) %>
          </div>
        </div>
  ...
  def handle_event("validate_email", %{"user" => %{"email" => email}}, socket) do
    ...
    {:noreply,
     assign(socket, user_params: user_params, email_valid: email_valid, form_valid: form_valid)}
  end

  @impl true
  def handle_event(
        "password_strength",
        %{"password" => password, "strength" => strength, "crack_time" => crack_time},
        socket
      ) do
    password_class =
      if strength == 100,
    ...
    # Updated line
    user_params = Map.merge(socket.assigns.user_params, %{password: password})

    form_valid =
      form_valid?(socket.assigns.email_valid, strength, socket.assigns.user_params[:name])
  ...
  def handle_event("email-taken", %{"message" => message}, socket) do
    ...
    {:noreply,
     ...
     # Reset user_params or handle as needed
     |> assign(:user_params, %{})}
  end
